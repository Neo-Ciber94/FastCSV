...
namespace FastCSV.Converters.Tests
{
    ...
    class QueueConverterTests
    {
        ...
        {
            var container = new Container(new Queue(new object[]{ 1, true, "hello", 24.05f, 'D', 3500.53m }), 6);
            string result = CsvConverter.Serialize(container, Options);

            Assert.AreEqual("item1,item2,item3,item4,item5,item6,Count\n1,true,hello,24.05,D,3500.53,6", result);
        }
        ...
        public void DeserializeTest()
        {
            ...
            CollectionAssert.AreEqual(new Queue(new object[] { 1, true, "hello", 24.05f, 'D', 3500.53m }), result.Items);
        }

        record Container(Queue Items, int Count);
    }
}
