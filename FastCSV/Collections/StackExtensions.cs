...
namespace FastCSV.Collections
{
    ...
    public static class StackExtensions
    {
        ...
        /// <param name="stack">The stack.</param>
        /// <param name="items">The items to push.</param>
        public static void PushRange<T>(this in ValueStack<T> stack, IEnumerable<T> items)
        {
            foreach (var e in items)
            {
                stack.Push(e);
        ...
        /// <param name="stack">The stack.</param>
        /// <param name="items">The items to pusems"></param>
        public static void PushRangeReverse<T>(this in ValueStack<T> stack, IEnumerable<T> items)
        {
            if (items is IList<T> list)
            {
                for (int i = list.Count - 1; i >= 0; i--)
            ...
            {
                PushRange(in stack, items.Reverse());
            }
        }
    }
}
